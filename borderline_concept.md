...
| Shared agency | Every player owns one independent order. Spoken deals have no enforcement UI; different attackers never combine their strength. Public remaining strength cards make threats assessable. |
...
| Fun hypothesis | Players make and reinterpret deals, deliberately choose when to use stronger cards, and understand the reveal well enough to react to one another. |
...
| Animation | Orders flip together; short order ribbons clearly originate at faction emblems/home ports, not fictional departing garrisons. Battles share a timeline; contested provinces receive brief emphasis. Flags change only on resolution, then scores visibly bank. |
...
- An invasion may target any non-owned province bordering an owned province, or either home entry province. A guard may target any owned province. No player chooses a source, moves a standing army, or removes soldiers from an origin.
...
- Territory has **zero** passive defense. Guarding supplies exactly the chosen strength for this turn. All played strength expires after resolution; ownership persists.
...
5. Apply all ownership changes together, bank each player's territory score, expire all played strength, and refresh cards if the three-turn cycle ended.
...
Public shorthand: **“Unguarded land can be taken. Highest strength wins; a tie for strongest leaves the flag where it was.”** The teaching sequence must demonstrate both defender ties and ties between attackers.
...
- Teach: strength 1 takes unguarded land; guard 2 stops strength 1; equal strongest attackers keep the flag; practice and card refill; permanent port recovery. Practice changes neither scores nor real starting state.
