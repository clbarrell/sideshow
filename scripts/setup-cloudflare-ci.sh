...
# Replace the example stage with the setup journey for this repository.
# ──────────────────────────────────────────────────────────────────────────
...
stage "GitHub access"
say "The wizard writes encrypted Actions secrets to the current GitHub repository."
if ! command -v gh >/dev/null 2>&1; then
  warn "GitHub CLI is not installed. Install it, then rerun this wizard."
  exit 1
fi
if gh auth status >/dev/null 2>&1; then
  say "GitHub CLI is already authenticated."
else
  step "Complete the GitHub login opened by the CLI."
...
step "Create the token and copy it now; Cloudflare only shows it once."
