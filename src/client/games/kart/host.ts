...
import { advanceCheckpoint, buildTrack, CHECKPOINT_EVERY as CP_EVERY, ROAD_HALF } from "./track";
  ...
  return {
    ...
    tick(elapsed) {
      ...
      for (const c of cars.values()) {
        ...
        c.driftSparkCool = Math.max(0, c.driftSparkCool - dt);
        if (c.finished !== null) {
          c.v *= 1 - 2.5 * dt;
        } else {
          ...
          const onBoostPad = track.boostPads.some((idx) => circularDistance(near.idx, idx, track.pts.length) <= 3);
          ...
          const handling = drive(c, c.input.s, c.input.t, c.boost > 0, offRoad, dt);
          if (handling.drifting && c.driftSparkCool <= 0) {
            const rearX = c.x - Math.cos(c.a) * 29;
            const rearY = c.y - Math.sin(c.a) * 29;
            burst(rearX, rearY, handling.charge > 0.65 ? "#FFC24A" : "#F6EFE2", 2, 105);
            c.driftSparkCool = 0.09;
          }
          if (handling.miniBoost > 0) {
            c.boost = Math.max(c.boost, handling.miniBoost);
            burst(c.x - Math.cos(c.a) * 30, c.y - Math.sin(c.a) * 30, "#FFC24A", 7, 215);
            markAudio(c.id, { boost: true });
            sound?.boost();
          }

          // Checkpoint / lap progress
          const advanced = advanceCheckpoint(track, c.lap, c.cp, c.x, c.y);
          if (advanced.crossed) {
            c.cp = advanced.checkpoint;
            if (advanced.lap !== c.lap) {
              c.lap = advanced.lap;
              if (c.lap === LAPS - 1) {
      ...
      finalCountdown.update(over ? null : remainingRaceTime(clock, firstFinish));
    },
    ...
    render(g, w, h) {
      ...
      for (const idx of track.boostPads) {
      ...
      for (const c of standings().reverse()) {
        ...
        g.beginPath();
        g.ellipse(1, 6, 40, 25, 0, 0, Math.PI * 2);
        g.fill();
        if (c.cool <= 0 && c.boost <= 0 && c.finished === null) {
          g.fillStyle = "rgba(255,194,74,0.24)";
          g.beginPath();
          g.ellipse(-34, 0, 18, 13, 0, 0, Math.PI * 2);
          g.fill();
          g.fillStyle = "rgba(255,239,184,0.7)";
          g.fillRect(-35, -9, 5, 6);
          g.fillRect(-35, 3, 5, 6);
        }
        if (c.boost > 0) {
        ...
        g.translate(c.x, c.y);
        g.rotate(c.a);
        g.globalAlpha = countdown > 0 ? 0.38 : clamp(0.26 - clock * 0.055, 0.055, 0.26);
        ...
        // Names already have a dedicated standings rail. A compact seat badge
        ...
        g.fill();
        g.fillStyle = c.color;
        g.fillRect(c.x - badgeSize / 2, c.y - labelLift - badgeSize, 4 * entityScale, badgeSize);
        g.fillStyle = "#F6EFE2";
        g.textAlign = "center";
      ...
      board.forEach((c, i) => {
        ...
        if (denseTicker) {
          ...
          g.fillText(`#${c.seat + 1}`, x + pillWidth / 2 - 4 * hudScale, tickerY + tickerHeight / 2 - 10 * hudScale);
          g.textAlign = "center";
          ...
          g.fillText(`#${c.seat + 1}`, x + 50 * hudScale, tickerY + tickerHeight / 2);
          g.font = `750 ${Math.round(21 * hudScale)}px Archivo, system-ui, sans-serif`;
          g.fillText(
            fitText(g, c.name, Math.max(0, pillWidth - 88 * hudScale)),
            x + 78 * hudScale,
...
export function remainingRaceTime(clock: number, firstFinish: number | null) {
